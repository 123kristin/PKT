...
def main(params):
    ...
    if model_name == 'PKT':
        testauc, testacc, testaucprc = evaluate(model, test_loader, model_name, save_test_path)
        print(f"testauc: {testauc}, testacc: {testacc}, testaucprc: {testaucprc}")
    else:
        testauc, testacc = evaluate(model, test_loader, model_name, save_test_path)
        print(f"testauc: {testauc}, testacc: {testacc}")
        

    window_testauc, window_testacc, window_testaucprc = -1, -1, -1
    save_test_window_path = os.path.join(save_dir, model.emb_type+"_test_window_predictions.txt")
    if model_name == 'PKT':
        window_testauc, window_testacc, window_testaucprc = evaluate(model, test_window_loader, model_name, save_test_window_path)
        print(f"testauc: {testauc}, testacc: {testacc}, testaucprc: {testauc}, window_testauc: {window_testauc}, window_testacc: {window_testacc}, window_testaucprc: {window_testaucprc}")
    else:
    ...
    if model_name == 'PKT':
        dres = {
        "testauc": testauc, "testacc": testacc, "testaucprc": testaucprc, "window_testauc": window_testauc, "window_testacc": window_testacc, "window_testaucprc": window_testaucprc,
    }  
    else:
        dres = {
    ...
    if model_name == 'PKT':
        q_testauc, q_testacc, q_testaucprc = -1, -1, -1
        qw_testauc, qw_testacc, qw_testaucprc = -1, -1, -1
        if "test_question_file" in data_config and not test_question_loader is None:
            ...
            q_testaucs, q_testaccs, q_testaucprcs = evaluate_question(model, test_question_loader, model_name, fusion_type, save_test_question_path)
            for key in q_testaucs:
                dres["oriauc"+key] = q_testaucs[key]
            for key in q_testaccs:
                dres["oriacc"+key] = q_testaccs[key]
            for key in q_testaucprcs:
                dres["oriaucprc"+key] = q_testaucprcs[key]
        ...
        if "test_question_window_file" in data_config and not test_question_window_loader is None:
            ...
            qw_testaucs, qw_testaccs, qw_testaucprcs = evaluate_question(model, test_question_window_loader, model_name, fusion_type, save_test_question_window_path)
            for key in qw_testaucs:
                dres["windowauc"+key] = qw_testaucs[key]
            for key in qw_testaccs:
                dres["windowacc"+key] = qw_testaccs[key]
            for key in qw_testaucprcs:
                dres["windowaucprc"+key] = qw_testaucprcs[key]
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="saved_model/assist2009_PKT_qid_saved_model_new_42_0_256_0.001_0_1_2_4_False_0.9_0.7_0.1_0.4_GRU_64")
    # parser.add_argument("--save_dir", type=str, default="saved_model/assist2012_PKT_qid_saved_model_new_42_0_8_0.003_0_1_2_2_False_0.2_0.1_0.2_0.4_GRU_64")
    # parser.add_argument("--save_dir", type=str, default="saved_model/assist2015_PKT_qid_saved_model_new_42_0_4_0.001_0_1_2_2_False_0.3_0.5_0.5_0.5_GRU_64")
    # parser.add_argument("--save_dir", type=str, default="saved_model/nips_task34_PKT_qid_saved_model_new_42_0_256_0.001_0_1_2_4_False_0.7_0.6_0.5_0.25_GRU_64")
