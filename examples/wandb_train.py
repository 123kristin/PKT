...
def main(params):
    ...
    if model_name == "rkt":
        ...
        testauc, testacc, testaucprc, window_testauc, window_testacc, window_testaucprc, validauc, validacc, validaucprc, best_epoch = train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, None, None, save_model)
    else:
    ...
    if model.model_name == 'PKT':
        print("fold\tmodelname\tembtype\ttestauc\ttestacc\ttestaucprc\twindow_testauc\twindow_testacc\twindow_testaucprc\tvalidauc\tvalidacc\tvalidaucprc\tbest_epoch")
        print(str(fold) + "\t" + model_name + "\t" + emb_type + "\t" + str(round(testauc, 4)) + "\t" + str(round(testacc, 4))+ "\t"  + str(round(testaucprc, 4)) + "\t" + str(round(window_testauc, 4)) + "\t" + str(round(window_testacc, 4)) + "\t" + str(round(window_testaucprc, 4)) + "\t" + str(validauc) + "\t" + str(validacc) + "\t" + str(validaucprc) +  "\t" + str(best_epoch))
    else:
    ...
    model_save_path = os.path.join(ckpt_path, emb_type+"focal_model.ckpt")
