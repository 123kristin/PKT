...
class IEKTNet(nn.Module): 
    def __init__(self, num_q,num_c,emb_size,max_concepts,lamb=40,n_layer=1,cog_levels=10,acq_levels=10,dropout=0,gamma=0.93, emb_type='qc_merge', emb_path="", pretrain_dim=768,device='cpu'):
        ...
        self.prob_emb = nn.Parameter(torch.randn(num_q, emb_size).to(self.device), requires_grad=True)
        self.gamma = gamma
        self.lamb = lamb
        self.gru_h = mygru(0, emb_size * 4, emb_size)
        self.concept_emb = nn.Parameter(torch.randn(self.concept_num, emb_size).to(self.device), requires_grad=True)#
...
class IEKT(QueBaseModel):
    ...
    def train_one_step(self,data,device, process=True):
        # self.step+=1
        # debug_print(f"step is {self.step},data is {data}","train_one_step")
        # debug_print(f"step is {self.step}","train_one_step")
        BCELoss = torch.nn.BCEWithLogitsLoss()
        
        data_new,emb_action_list,p_action_list,states_list,pre_state_list,reward_list,predict_list,ground_truth_list = self.predict_one_step(data,device, return_details=True,process=process)
        data_len = data_new['cc'].shape[0]
        seq_len = data_new['cc'].shape[1]

    
        seq_num = torch.where(data['qseqs']!=0,1,0).sum(axis=-1)+1
        emb_action_tensor = torch.stack(emb_action_list, dim = 1).to(device) 
        p_action_tensor = torch.stack(p_action_list, dim = 1).to(device)
        state_tensor = torch.stack(states_list, dim = 1).to(device)
        pre_state_tensor = torch.stack(pre_state_list, dim = 1).to(device)
        reward_tensor = torch.stack(reward_list, dim = 1).float() / (seq_num.unsqueeze(-1).repeat(1, seq_len)).float().to(device) #equation15
        logits_tensor = torch.stack(predict_list, dim = 1).to(device)
        ground_truth_tensor = torch.stack(ground_truth_list, dim = 1).to(device)
        loss = []
        tracat_logits = []
        tracat_ground_truth = []
    ...
    def predict_one_step(self,data,device, return_details=False,process=True):
        sigmoid_func = torch.nn.Sigmoid()
        data_new = self.batch_to_device(data,device, process)
        ...
        for seqi in range(0, seq_len):#
            ...
            emb_action_list.append(emb_a)#s_t 
            p_action_list.append(emb_ap)#m_t
            states_list.append(out_x)
            pre_state_list.append(ques_h)#上
