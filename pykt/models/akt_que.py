...
class AKTQueNet(nn.Module):
    def __init__(self, num_q, num_c, emb_size, n_blocks, dropout, d_ff=256, 
        ...
        if self.num_q > 0:
            ...
            self.q_embed_diff = nn.Embedding(self.num_q+1, emb_size) 
            self.qa_embed_diff = nn.Embedding(2 * self.num_q + 1, emb_size) 
        
        if self.separate_qa: 
            self.qa_embed = nn.Embedding(2*self.num_c+1, emb_size) 
        else: # false default
    ...
    def base_emb(self, q, c, r):
        q_embed_data = self.que_emb(q,c,r)  
        if self.separate_qa:
