...
# 版本3
# def cal_loss(model, ys, y, y1, r, rshft, sm, preloss=[]):
    ...
    # 版本2
        ...
        y = torch.masked_select(ys[0], sm)
        y1 = torch.masked_select(ys[1], sm)
        t = torch.masked_select(rshft, sm)

        loss1 = binary_cross_entropy(y.double(), t.double())
        loss2 = binary_cross_entropy(y1.double(), t.double())
        
        # assist2009
        loss_func = FocalLoss(gamma=2, alpha=1.82) # avg 

        # assist2012
        # loss_func = FocalLoss(gamma=2.7, alpha=2.7) # avg
      
        
        # assist2015
        # loss_func = FocalLoss(gamma=2, alpha=2.73) # avg

        # nips
        # loss_func = FocalLoss(gamma=2, alpha=1.2) # 200better
        # loss_func = FocalLoss(gamma=2, alpha=1.16) # 200用这个
        ...
        loss = loss1 + lambda_rc * loss2 + lambda_cb * loss3
...
def model_forward(model, data, rel=None):
    ...
    if model_name in ["atdkt"]:
        ...
        y, y2, y3 = model(dcur, device, train=True) 
    ...
    if model_name in ["dkt"]:
        ...
        print("y的shape + ", y.shape) # 【256,199,123】
        y = (y * one_hot(cshft.long(), model.num_c)).sum(-1)
        print(y.shape) # [256,199]
        ys.append(y) # first: yshft

    elif model_name == 'PKT':
        # loss_sim, y= model.stepTrain(c.long(), r.long(), m.long(), device)  
        # 加入q
        loss_sim, y= model.stepTrain(c.long(), q.long(), r.long(), m.long(), device)   
    
        y = (y * one_hot(cshft.long(), model.num_c)).sum(-1)
    

    
        y1 =(loss_sim * one_hot(cshft.long(), model.num_c)).sum(-1)
    ...
    # cal loss
        ...
        y,loss = model.train_one_step(data, device) 
...
def train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, test_loader=None, test_window_loader=None, save_model=False, data_config=None, fold=None):
    ...
    for i in range(1, num_epochs + 1):
        ...
        elif model.model_name == 'PKT':
            auc, acc, aucprc = evaluate(model, valid_loader, model.model_name)
        else:
        ...
        if auc > max_auc+1e-3:
            ...
            max_auc = auc
            best_epoch = i
            testauc, testacc, testaucprc = -1, -1, -1
            ...
            if not save_model:
                if test_loader != None:
                    ...
                    if model.model_name == 'PKT':
                        testauc, testacc, testaucprc = evaluate(model, test_loader, model.model_name)
                    else:
                ...
                if test_window_loader != None:
                    ...
                    if model.model_name == 'PKT':
                        window_testauc, window_testacc, window_testaucprc = evaluate(model, test_window_loader, model.model_name)
                    else:
                        window_testauc, window_testacc = evaluate(model, test_window_loader, model.model_name, save_test_path)
            if model.model_name == 'PKT':
                validauc, validacc, validaucprc = auc, acc, aucprc
            else:
                validauc, validacc = auc, acc
        if model.model_name == 'PKT':
             print(f"Epoch: {i}, validauc: {validauc:.4}, validacc: {validacc:.4}, validaucprc: {validaucprc:.4},best epoch: {best_epoch}, best auc: {max_auc:.4}, train loss: {loss_mean}, emb_type: {model.emb_type}, model: {model.model_name}, save_dir: {ckpt_path}")
             print(f"            testauc: {round(testauc,4)}, testacc: {round(testacc,4)}, testaucprc: {round(testaucprc,4)}, window_testauc: {round(window_testauc,4)}, window_testacc: {round(window_testacc,4)}, window_testaucprc: {round(window_testaucprc,4)}, ")
        else:
        ...
        if i - best_epoch >= 10:
            break
    if model.model_name == 'PKT':
        return testauc, testacc, testaucprc, window_testauc, window_testacc, window_testaucprc, validauc, validacc, validaucprc, best_epoch
    else:
