    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["atdkt"]:
                ...
                y = model.predict_one_step(data, device) 
                c,cshft = q,qshft#question level 
        ...
        aucprc = metrics.average_precision_score(ts, ps)

  
    if model_name == 'PKT':
        return auc, acc, aucprc
    else:
        return auc, acc
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dkvmn","skvmn"]:
                ...
                y, h = model(dcurori, device, qtest=True, train=False)
                y = y[:,1:]
            elif model_name in ["akt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
                ...
                y = model(dcurori, device)#c.long(), r.long(), q.long())
                y = (y * one_hot(cshft.long(), model.num_c)).sum(-1)
            elif model_name in ["dkt", "dkt+"]:
                y = model(c.long(), r.long())
                y = (y * one_hot(cshft.long(), model.num_c)).sum(-1)
        ...
        if model_name == 'PKT':
            # ori concept eval
            aucs, accs, aucprcs = dict(), dict(), dict()
            ts = np.concatenate(y_trues, axis=0)
            ps = np.concatenate(y_scores, axis=0)
