...
class simpleKT(nn.Module):
    ...
    def base_emb(self, q_data, target):
        q_embed_data = self.q_embed(q_data)  # BS, seqlen,  d_model# c_ct  z_{c_k} = W_c.e_{c_k}
        if self.separate_qa:
    ...
    def forward(self, dcur, device, qtest=False, train=False):
        ...
        if self.n_pid > 0 and emb_type.find("norasch") == -1: # have problem idm  ,,,,,,,,,
            if emb_type.find("aktrasch") == -1:
                ...
                q_embed_data = q_embed_data + pid_embed_data * \
                    q_embed_diff_data  # uq *d_ct + c_ct # question encoder  x_t = z_{c_k} + m_{q_j}.v_{c_k}
