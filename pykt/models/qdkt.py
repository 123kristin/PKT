...
class QDKTNet(nn.Module):
    ...
    def forward(self, q, c ,r,data=None):
        x = (q + self.num_q * r)[:,:-1]  [32,199]
        print(x.shape) # torch.Size([32, 87])
        xemb = self.interaction_emb(x)
        print(xemb.shape) # torch.Size([32, 87, 300])
...
class QDKT(QueBaseModel):
    ...
    def train_one_step(self,data,device, process=True,return_all=False):
        outputs,data_new = self.predict_one_step(data,device, return_details=True,process=process) 
        loss = self.get_loss(outputs['y'],data_new['rshft'],data_new['sm'])  
        return outputs['y'],loss#y_question没用

    def predict_one_step(self,data,device, return_details=False,process=True,return_raw=False):
        data_new = self.batch_to_device(data,device, process=process)
